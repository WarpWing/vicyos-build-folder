...
###################################################################################
######  Adapting the Output to be compatible with VicyosLinux-hybrid-hlwm:  #######
###################################################################################


# Set the Iso Label, Name, Publisher, Aplication, and ISO version!
sed -i "s/iso_name=/iso_name=vicyos-hybrid-hlwm/g" work/archiso/build.sh
sed -i "s/iso_label=/iso_label="vicyos-hybrid-hlwm-$(date +%Y.%m.%d)"/g" work/archiso/build.sh
sed -i "s/iso_publisher="'""'"/iso_publisher="'"vicyos-hybrid-hlwm <https:\/\/github.com\/felipendc>"'"/g" work/archiso/build.sh
sed -i "s/iso_application="'""'"/iso_application="'"vicyos-hybrid-hlwm Live\/Rescue CD"'"/g" work/archiso/build.sh
sed -i "s/iso_version=/iso_version="$(date +%Y.%m.%d)"/g" work/archiso/build.sh

# Add the build date to the "DISTRIB_RELEASE"
release=$(date +%Y.%m.%d)
sed -i "s/DISTRIB_ID=VicyosLinux/DISTRIB_ID=VicyosLinux-hybrid-hlwm-$release/g" work/archiso/airootfs/etc/vicyos-release
...
sed -i "s/ISO_CODENAME=VicyosLinux/ISO_CODENAME=VicyosLinux-hybrid-hlwm-$release/g" work/archiso/airootfs/etc/dev-rel
sed -i "s/ISO_RELEASE=/ISO_RELEASE=$release/g" work/archiso/airootfs/etc/dev-rel

# Change the Grub Distributor
sed -i "s/GRUB_DISTRIBUTOR="'"VicyosLinux"'"/GRUB_DISTRIBUTOR="'"VicyosLinux-hybrid-hlwm"'"/g" work/archiso/airootfs/etc/default/grub
